...
This study examines whether interpretable signals derived from historical prices, returns and trading volume can explain cross-sectional differences in future returns among point-in-time S&P500 constituents. The dataset covers 2008–2026, with observations from 2008–2009 retained as factor warm-up history and formal research beginning in 2010. Historical index membership is used to reduce survivorship bias, although incomplete Yahoo Finance coverage for old and delisted securities leaves residual data-availability limitations.

The research evaluates 11 factor families, 56 parameter configurations and eight forward-return horizons. Their combination produces 448 factor-horizon hypotheses and 2,240 statistical and economic effect tests. Factor relationships are evaluated through cross-sectional Rank IC, quantile-return structures, HAC-adjusted inference, temporal consistency and global false-discovery-rate correction. Four explicitly labelled leads are subsequently examined through portfolio implementations incorporating alternative position-construction rules, rebalance frequencies, transaction costs, turnover, beta exposure, walk-forward behaviour and point-in-time market regimes.

One volatility-scaled momentum Rank-IC result survives global FDR correction, but no directly interpretable economic return effect passes the complete Factor Selection rules. The strongest portfolio-level result is a 60-day Low Volatility implementation, which produces a 4.77% net annualized return, a 0.505 full-history Sharpe ratio and a 0.432 long walk-forward Sharpe ratio at the declared 10 bps transaction-cost assumption. However, its alpha HAC t-statistic is 1.60, and neighbouring lookback windows do not form a robust parameter plateau.

The study therefore finds no validated alpha. Its main contribution is a reproducible point-in-time factor-research framework, together with one economically interesting but isolated Low Volatility lead and a transparent account of the statistical, economic and data limitations preventing a stronger conclusion.
...
| Strongest economic lead | Low Volatility 60d |
| Net annualized return at 10 bps | 4.77% |
| Full-history Sharpe | 0.505 |
| Long walk-forward Sharpe | 0.432 |
| Final result | No validated alpha |


## 1. Introduction

The practical objective of this project is to determine whether simple and interpretable information already contained in equity prices, returns and trading volume can rank stocks by their subsequent return. The object being predicted is not the direction of the S&P 500. It is the relative performance of securities inside the index universe on the same date.

This distinction defines the complete research design. On every date, each available stock receives a factor score. The cross-section is then examined to determine whether stocks with different scores subsequently earn systematically different returns. A useful factor must therefore show more than a large historical average. Its direction and economic shape should persist through time, survive correction for the number of hypotheses tested and remain usable after portfolio construction, turnover and transaction costs.

The project deliberately focuses on transparent factors rather than a black-box forecasting model. This makes every result traceable to an economic idea, a formula and a point-in-time set of securities. It also makes negative findings informative: a factor can be rejected because it lacks a stable cross-sectional relationship, because an apparent effect disappears under multiple-testing correction, or because the relationship cannot be implemented economically.

The final result is negative in the strict sense. No tested factor qualifies as validated alpha. One volatility-scaled momentum Rank-IC result survives the global statistical correction, and a 60-day Low Volatility portfolio becomes the strongest economic lead. Neither satisfies the complete confirmation standard. The project is therefore presented as a completed factor-research study, not as a production-ready trading strategy.


## 2. Research Questions and Hypotheses

The study addresses five ordered research questions.

1. Do any of the tested factor scores contain a stable cross-sectional relationship with future stock returns?
2. Does the relationship have an economically interpretable shape, such as monotonicity, an upper-tail effect or a lower-tail effect?
3. Does the evidence remain statistically visible after accounting for the complete set of 448 factor-horizon hypotheses and 2,240 effect tests?
4. Can a frozen lead be converted into a portfolio that remains useful after turnover, transaction costs and basic market-risk controls?
5. Is the portfolio result stable across time, rebalance dates, market regimes and nearby parameter values?

These questions imply the following hypotheses.

- **H1 — Cross-sectional information:** at least one factor configuration is associated with the ordering of subsequent returns.
- **H2 — Economic structure:** at least one hypothesis produces a stable monotonic or tail-return pattern rather than an isolated average correlation.
- **H3 — Multiple-testing robustness:** at least one economic effect survives the global false-discovery-rate procedure applied to the complete declared test family.
- **H4 — Implementability:** at least one frozen lead produces positive and sufficiently stable net portfolio performance under the declared 10 bps transaction-cost assumption.
- **H5 — Parameter robustness:** a successful implementation is supported by neighbouring factor parameters rather than one isolated historical maximum.

The sign of a factor is not assumed to be positive. A strong negative relationship can be as useful as a positive one because the portfolio direction can be reversed. The relevant questions are the magnitude, stability and economic meaning of the relationship.


## 3. Economic Motivation and Literature

The factor universe combines established return-predictability ideas with closely related transformations. The purpose is not to reproduce every published anomaly. It is to test a compact set of understandable mechanisms using one consistent point-in-time research architecture.

### Momentum, trend and proximity to highs

Momentum asks whether securities that performed strongly relative to other securities continue to outperform. The classic evidence of [Jegadeesh and Titman (1993)](https://doi.org/10.1111/j.1540-6261.1993.tb04702.x) motivates medium-horizon winner-minus-loser tests. The project varies the formation period and the recent period omitted from that formation window. Trend, trend slope and proximity to a rolling high test related but non-identical descriptions of persistent price movement.

Momentum can arise from slow information diffusion, underreaction or institutional trading persistence. It can also reverse, decay or become crowded. The project therefore does not treat one conventional 12-1 specification as automatically valid. It compares multiple horizons, residual versions, volatility-scaled versions and price-volume confirmation.

### Short-term reversal

Short-term reversal tests whether recent relative winners become near-term losers and recent losers rebound. [Jegadeesh (1990)](https://doi.org/10.1111/j.1540-6261.1990.tb05110.x) documents negative first-order serial correlation and motivates treating reversal as a separate family rather than merely using a negative momentum coefficient. A reversal effect may reflect temporary liquidity pressure, bid-ask effects or short-lived overreaction; it may therefore disappear once turnover and costs are introduced.

### Low volatility and risk-adjusted signals

Low Volatility gives a higher factor score to stocks with lower trailing return variability. The sign is explicitly reversed so that a larger score always represents the preferred side of this factor definition. The literature documents that high-volatility and high-beta stocks have often failed to deliver proportionately higher returns; [Baker, Bradley and Wurgler (2011)](https://pages.stern.nyu.edu/~jwurgler/papers/faj-benchmarks.pdf) discuss benchmark and leverage constraints as one possible explanation.

This project also tests volatility-scaled momentum and risk-adjusted trend. These signals ask whether a given price movement is more informative when achieved with less realised variability. They are transformations of price behaviour, not independent fundamental information.

### Liquidity and price-volume interaction

Liquidity can affect both expected returns and implementation costs. [Amihud (2002)](https://doi.org/10.1016/S1386-4181(01)00024-6) relates illiquidity to expected returns using price and dollar-volume information, while [Lee and Swaminathan (2000)](https://doi.org/10.1111/0022-1082.00280) connect past trading volume with the magnitude and persistence of momentum. The current project uses changes in rolling dollar volume and a price-volume confirmation transformation. These are deliberately simple proxies and should not be interpreted as full market-impact models.

### Why strict inference is required

Testing many factor definitions and horizons creates a large chance of selecting a lucky historical result. [Harvey, Liu and Zhu (2016)](https://doi.org/10.1093/rfs/hhv059) show why conventional single-test significance thresholds are inadequate in a crowded factor-discovery setting. The project therefore separates descriptive raw p-values from the confirmatory global false-discovery-rate decision and uses the [Benjamini–Hochberg procedure](https://doi.org/10.1111/j.2517-6161.1995.tb02031.x). Because daily observations and forward returns overlap, reported inference uses heteroskedasticity- and autocorrelation-consistent standard errors following [Newey and West (1987)](https://www.nber.org/papers/t0055).
...
The market-data matrices contain 4,686 trading dates from 2008-01-02 through 2026-08-18 and 900 historical ticker columns. Observations from 2008–2009 are retained as warm-up history for long rolling calculations. Formal factor evaluation begins on 2010-01-01.

The equity inputs are adjusted prices and raw trading volume downloaded through Yahoo Finance. The stored volume is rescaled so that adjusted price multiplied by compatible volume equals raw close multiplied by raw Yahoo volume. Daily returns and forward returns are calculated from adjusted prices. The risk-free series is the daily US three-month Treasury constant-maturity rate, DGS3MO, obtained from the [Federal Reserve Bank of St. Louis](https://fred.stlouisfed.org/series/DGS3MO).

### Point-in-time membership

Using only the current S&P 500 list would create direct survivorship bias. The project instead reconstructs date-specific membership from 2,720 historical component snapshots supplied by the community-maintained [fja05680/sp500 dataset](https://github.com/fja05680/sp500/blob/master/S%26P%20500%20Historical%20Components%20%26%20Changes.csv). The source contains between 496 and 507 components per snapshot. A stock is eligible only when it is an index member on that date, a price is observed and the price passes the data-quality mask.

This approach reduces survivorship bias but does not eliminate it. Yahoo Finance has incomplete or missing histories for many old, renamed and delisted securities. In the final audit, 247 historical tickers have no price during their recorded membership, 262 have membership-period price coverage below 80%, 191 downloads are missing and 20 reused symbols are deliberately rejected. The research universe is therefore point-in-time with respect to the membership source, but not a complete institutional-grade delisting database.

### Data quality

The final data audit reports 54 PASS, four WARNING and zero FAIL checks. It verifies matrix alignment, formulas, membership reconstruction, valid prices, returns, adjusted-volume compatibility, availability and risk-free-rate coverage. Large returns are not winsorized or clipped. Unconfirmed price anomalies are quarantined through the quality mask, while confirmed real events remain usable.

Volume quality is kept separate from price availability. Thirty-three zero-volume observations are excluded from liquidity calculations without removing the corresponding price observations. Thirty-six positive-to-positive volume jumps of at least 100 times remain as warnings because an internal consistency check cannot determine whether a positive market observation is economically genuine.

### Information timing

Daily return at date `t` means the adjusted-price return from `t-1` to `t`. A forward return for horizon `h` is

\[
R_{i,t}^{(h)} = \frac{P_{i,t+h}}{P_{i,t}} - 1.
\]

The factor used for a relationship dated `t` is shifted by one trading date. Factor Selection therefore compares information known at `t-1` with the return from `t` to `t+h`. Portfolio weights entering date `t` also use only the factor known at `t-1`, before earning the `t-1` to `t` daily return.
...
The research contains 11 factor families and 56 fixed configurations.

| Family | Configurations | Economic interpretation |
| --- | ---: | --- |
| Momentum | 17 | Cumulative historical return over different formation and skip periods |
| Low Volatility | 7 | Negative trailing return volatility, so lower volatility receives a higher score |
| Trend | 6 | Current price relative to its simple moving average |
| Short-Term Reversal | 3 | Negative recent cumulative return |
...
| High Proximity | 3 | Current price relative to its rolling high |
| Trend Slope | 4 | Annualized rolling slope of log price |
| Risk-Adjusted Trend | 4 | Trend slope divided by annualized volatility |
| Liquidity Change | 3 | Log ratio of short- and long-window average dollar volume |
| Price-Volume Confirmation | 3 | Momentum adjusted by a bounded liquidity-change signal |

Each raw factor is restricted to the valid point-in-time availability matrix and transformed into a same-date cross-sectional z-score:

\[
z_{i,t} = \frac{x_{i,t}-\bar{x}_t}{s_t}.
\]

Consequently, a score of zero means that the stock is close to the same-date cross-sectional mean of that factor, not that the stock had zero return or zero trend. Winsorization is implemented as an optional transformation but is disabled in the completed research, so verified extreme factor values remain in the analysis.

### Horizons and hypotheses

Eight forward-return horizons are used: 1, 5, 10, 21, 42, 63, 126 and 252 trading days. Combining 56 configurations with eight horizons creates 448 factor-horizon hypotheses. A horizon changes the future return being compared with the factor; it does not change the factor score itself.

### Daily cross-sectional evidence

For each date and hypothesis, eligible stocks are ordered by their lagged factor score and divided into ten quantiles. At least 30 stocks with both a factor and a forward return are required. The system stores the number of securities, factor-score mean and median, and future-return mean and median for every quantile.

The daily Rank Information Coefficient is the cross-sectional Spearman correlation

\[
IC_t^{(h)} = \rho_{S}\left(z_{i,t-1}, R_{i,t}^{(h)}\right).
\]

It answers whether higher factor ranks are associated with higher or lower future-return ranks across stocks on that date. It does not represent an IC for one individual stock and it does not by itself specify a trading portfolio.

The quantile curves provide the economic shape that IC alone can hide. Five effects are tested for every hypothesis: Rank IC, Q10 minus Q1, Q10 minus the middle quantiles, middle quantiles minus Q1, and both edges minus the middle. This allows positive monotonic, negative monotonic, upper-tail, lower-tail and two-sided tail patterns to be distinguished.


## 6. Experimental Design

### Stage 1 — Complete hypothesis grid

The Factor Layer creates all 56 factor-score matrices and all eight forward-return matrices before any winning configuration is known. The Factor Selection Layer then evaluates the complete 448-hypothesis grid. The declared configuration is stored in code and run metadata, preventing the final report from silently omitting weak variants.

### Stage 2 — Statistical and economic classification

Every hypothesis receives daily IC and quantile-return observations. HAC-adjusted statistics account for serial dependence, including the overlap created by multi-day forward returns. Monthly and annual direction rates measure whether an effect is repeatedly present rather than produced by one subperiod. Rank autocorrelation describes factor persistence and therefore has implications for turnover.

Pattern classification requires sufficient observations, an economically recognisable quantile shape, temporal consistency and statistical visibility. The sign is not hard-coded: both positive and negative monotonic structures are admissible. The Benjamini–Hochberg correction is applied globally to all 2,240 declared effect tests. Exploratory within-group and hierarchical corrections are reported only as sensitivity analyses; they do not replace the original confirmatory rule after the results are visible.

### Stage 3 — Frozen portfolio leads

The complete selection procedure produces no final economic candidate. Four leads are nevertheless frozen for transparent post-selection investigation:

| Lead | Horizon | Reason retained |
| --- | ---: | --- |
| Volatility-Scaled Momentum 12m-1m / vol60 | 1 day | The only Rank-IC result surviving global FDR |
| Short-Term Reversal 5d | 5 days | Exploratory upper-tail pattern |
| Liquidity Change 20d/252d | 252 days | Exploratory upper-tail pattern |
| Low Volatility 60d | 63 days | Exploratory lower-tail pattern |

Freezing the list prevents the portfolio layer from repeatedly returning to the 448-hypothesis grid after observing later results. These are research leads, not confirmed discoveries.

### Stage 4 — Portfolio implementation

The four leads are tested through continuous ranks, Q10-Q1, Q10-middle, middle-Q1 and one-sided Q1 or Q10 portfolios. Market-neutral methods are tested with and without trailing beta neutralization. Rebalancing occurs every 1, 5, 21 or 63 trading days, with multiple calendar phases used to reduce dependence on one arbitrary starting date.

The engine carries weights through realised returns between rebalances. One-way turnover is the sum of absolute changes between target and pre-trade weights. Net daily return at cost `c` basis points is calculated as

\[
r_{p,t}^{net} = (1+r_{p,t}^{gross})\left(1-\frac{c}{10{,}000}\,Turnover_t\right)-1.
\]

The primary result uses 10 bps, with 0, 5 and 25 bps retained as sensitivity cases. Evaluation includes annualized return, volatility, Sharpe ratio, maximum drawdown, turnover, cost drag, market beta and HAC alpha. Long-only Sharpe ratios use returns above the daily risk-free rate; self-financing spreads use their own net return.

### Stage 5 — Temporal and parameter robustness

Walk-forward analysis uses two repeated structures: an 18-month selection window followed by six months of evaluation, and a four-year selection window followed by twelve months of evaluation. The implementation itself remains fixed; only the direction of a market-neutral spread can be chosen inside the preceding window. Because the candidates were discovered using the same full historical dataset, these windows are post-selection stability diagnostics, not a genuinely untouched final test.

The strongest lead is examined separately across Low Volatility windows of 20, 40, 60, 90, 120, 180 and 252 days. This final step asks whether success forms a coherent neighbouring parameter plateau or is concentrated at one isolated setting.


## 7. Factor Selection Results

The complete selection run analyzes 448 hypotheses and 2,240 effect tests. Of the 448 hypotheses, 426 show no stable structure. Twenty-two display a descriptive economic pattern: 14 lower-tail patterns, seven upper-tail patterns and one positive monotonic pattern. Ten of the 22 are temporally unstable and twelve are rejected by the global multiple-testing rule.

One volatility-scaled momentum Rank-IC result survives global FDR correction. This means the cross-sectional rank ordering contains statistically detectable information at its declared horizon. It does not establish a tradable return spread: no economic quantile effect survives the same complete global correction, and the volatility-scaled momentum Q10-Q1 relationship is not sufficiently supported.

The selection result therefore supports H1 only in a narrow statistical sense. H2 and H3 are not confirmed for an economic return effect. The difference matters: a significant average rank relationship can be too diffuse, unstable or economically small to produce a reliable portfolio.

![Factor Selection summary](Results/Factor_Selection_Layer/Figures/factor_selection_summary.png)


## 8. Portfolio Research Results

### General research

The general portfolio layer evaluates 160 implementations at the primary 10 bps cost assumption. Short-Term Reversal and Liquidity Change are rejected. Volatility-Scaled Momentum remains a statistical lead but its chosen implementation has a net annualized return of 2.33%, a Sharpe ratio of 0.268 and a long walk-forward Sharpe ratio of -0.008.

The 60-day Low Volatility implementation is the strongest economic lead. Its selected Q10-minus-middle, beta-neutral, 63-day rebalance portfolio produces:

| Metric | Result |
| --- | ---: |
| Net annualized return | 4.77% |
| Full-history Sharpe | 0.505 |
...
For context, the point-in-time equal-weight market proxy earns approximately 14.12% annually with a Sharpe ratio of 0.738 and a maximum drawdown of -39.90%, while the average annualized risk-free return is approximately 1.50%. The Low Volatility spread therefore has a lower absolute return than the equity benchmark but materially lower market exposure and drawdown. It should be judged as a relative, beta-controlled signal rather than as a replacement for a fully invested equity portfolio.

![General Research summary](Results/Research_Layer/General/Figures/general_research_summary.png)

### Focused Low Volatility research

The focused study holds the portfolio rule fixed and compares seven neighbouring lookback windows. The 60-day window passes six of ten declared checks and remains positive in both walk-forward structures. However, its alpha HAC t-statistic is below the declared significance threshold, its alpha Benjamini–Hochberg q-value is approximately 0.479 and nearby windows do not form a stable plateau that passes the same complete rules.

The 60-day result is therefore labelled `ISOLATED_RESULT`. It is economically interesting enough to document, but H4 and H5 are not jointly supported and the result is not upgraded to validated alpha.
...
The main lesson is not that price-based factors contain no information. The surviving volatility-scaled momentum Rank-IC result shows that the complete system can detect a statistically unusual cross-sectional ordering. The Low Volatility portfolio also shows that an economically coherent spread can persist after estimated trading costs and through repeated historical windows.

The stronger conclusion fails because the evidence does not align across every level. The statistically strongest rank result is not the economically strongest portfolio. The economically strongest portfolio has a modest Sharpe ratio, an alpha t-statistic below the declared threshold and weak support from neighbouring parameters. This separation is exactly why factor discovery, pattern classification and portfolio implementation are treated as different layers.

The Low Volatility lead may still have practical research value. Its drawdown and market-risk profile are different from those of the equal-weight benchmark, and the neighbouring windows are not uniformly poor. However, selecting the 60-day maximum after observing the complete history creates an additional selection problem. More tuning on the same data would increase confidence in the backtest appearance without supplying independent evidence.

The appropriate next confirmation would therefore use genuinely new information: future observations accumulated after the research freeze, a separate institutional-quality dataset with improved delisting history, or a pre-registered replication in a different equity universe. Additional parameter search on the current history is not equivalent to confirmation.


## 10. Threats to Validity

- **Residual survivorship and data-availability bias.** Historical membership is reconstructed, but Yahoo Finance does not provide complete histories for all old and delisted securities. Missing securities may change both factor ranks and realised returns.
- **Membership-source risk.** Historical components come from a community-maintained repository rather than official S&P point-in-time data.
- **No untouched final sample.** The complete history influenced factor discovery and the frozen lead set. Walk-forward results describe temporal behaviour after selection; they are not a new independent experiment.
- **Multiple testing.** Global FDR controls the declared family, but the broader human research process also contains choices about factor families, configurations, portfolio methods and follow-up studies.
- **Overlapping returns.** Multi-day forward returns share daily observations. HAC inference reduces, but does not eliminate, the difficulty of interpreting dependent tests.
- **Approximate implementation costs.** Fixed basis-point costs do not model bid-ask variation, nonlinear market impact, capacity, borrow fees, short availability or financing constraints.
- **Incomplete risk controls.** Point-in-time sector classifications are unavailable. The code reports sector exposure as `NOT_AVAILABLE` instead of incorrectly applying current sectors to historical companies.
- **Simple market model.** Alpha is evaluated against an equal-weight market proxy rather than a complete multi-factor risk model.
- **Regime-analysis selection.** Regime thresholds use only information available at the time, but regime comparisons are still post-selection analyses of already selected candidates.
- **Price-and-volume scope.** The factor set omits fundamentals, analyst estimates, options, intraday information and alternative data. The conclusion applies only to the tested signal universe.


## 11. Final Conclusion

This project tested whether interpretable price-, return- and volume-based signals could identify cross-sectional differences in future returns among point-in-time S&P 500 members.

The completed system covered 11 factor families, 56 configurations, eight horizons, 448 hypotheses and 2,240 effect tests. One volatility-scaled momentum Rank-IC result survived global FDR correction, but no directly interpretable economic return effect passed the complete Factor Selection rules. Four explicitly labelled leads were then frozen for post-selection portfolio research. Short-Term Reversal and Liquidity Change were rejected, and Volatility-Scaled Momentum remained only a statistical lead.

The 60-day Low Volatility implementation produced the strongest economic result: 4.77% net annualized return, a 0.505 full-history Sharpe ratio and a 0.432 long walk-forward Sharpe ratio under the declared 10 bps cost assumption. Its alpha HAC t-statistic of 1.60 and the absence of a robust neighbouring parameter plateau prevent it from being treated as confirmed alpha.

The final conclusion is therefore **no validated alpha**. The successful output of the project is a reproducible research framework, a transparent negative result and one clearly bounded Low Volatility lead suitable for independent future confirmation. The project should not be presented as a production-ready strategy or as evidence of guaranteed future returns.
...
- Baker, M., Bradley, B., and Wurgler, J. (2011). “Benchmarks as Limits to Arbitrage: Understanding the Low-Volatility Anomaly.” *Financial Analysts Journal*, 67(1), 40–54. [Author manuscript](https://pages.stern.nyu.edu/~jwurgler/papers/faj-benchmarks.pdf).
- Benjamini, Y., and Hochberg, Y. (1995). “Controlling the False Discovery Rate: A Practical and Powerful Approach to Multiple Testing.” *Journal of the Royal Statistical Society: Series B*, 57(1), 289–300. [DOI](https://doi.org/10.1111/j.2517-6161.1995.tb02031.x).
- Board of Governors of the Federal Reserve System. “Market Yield on U.S. Treasury Securities at 3-Month Constant Maturity, Quoted on an Investment Basis [DGS3MO].” [FRED](https://fred.stlouisfed.org/series/DGS3MO).
- Harvey, C. R., Liu, Y., and Zhu, H. (2016). “… and the Cross-Section of Expected Returns.” *The Review of Financial Studies*, 29(1), 5–68. [DOI](https://doi.org/10.1093/rfs/hhv059).
- Jegadeesh, N. (1990). “Evidence of Predictable Behavior of Security Returns.” *The Journal of Finance*, 45(3), 881–898. [DOI](https://doi.org/10.1111/j.1540-6261.1990.tb05110.x).
...
- Lee, C. M. C., and Swaminathan, B. (2000). “Price Momentum and Trading Volume.” *The Journal of Finance*, 55(5), 2017–2069. [DOI](https://doi.org/10.1111/0022-1082.00280).
- Newey, W. K., and West, K. D. (1987). “A Simple, Positive Semi-Definite, Heteroskedasticity and Autocorrelation Consistent Covariance Matrix.” *Econometrica*, 55(3), 703–708. [NBER working paper](https://www.nber.org/papers/t0055).
- fja05680. “S&P 500 Historical Components & Changes.” Community-maintained point-in-time membership source. [GitHub](https://github.com/fja05680/sp500/blob/master/S%26P%20500%20Historical%20Components%20%26%20Changes.csv).


# Technical Appendix

The appendix maps the research design to the exact files, functions, parameters and generated artifacts. It preserves the implementation-level documentation separately from the economic argument in the main paper.


## Appendix A — Project Architecture and Environment

### Technology Stack

- **Python** — complete research orchestration and reproducible layer pipelines.
- **pandas and NumPy** — point-in-time matrices, time series and cross-sectional calculations.
- **SciPy and statsmodels** — correlation tests, HAC inference, regressions and multiple-testing correction.
- **yfinance and Requests** — market, historical-universe and risk-free-rate data acquisition.
- **PyArrow and Parquet** — compressed storage and filtered access to large research matrices.
- **Matplotlib** — automatically generated audit, factor-selection and portfolio-research figures.
...
## Appendix B — Data System Implementation
...
### Data System Output Verification
...
## Appendix C — Factor Layer Implementation
...
### Factor Layer Output Verification
...
## Appendix D — Factor Selection Implementation
...
### Factor Selection Output Verification
...
- `factor_selection_summary.png`: the compact scope and final-decision summary used in this research paper.
...
## Appendix E — Research Layer Implementation
...
### Research Layer Output Verification
...
## Appendix F — Complete Pipeline, Testing and Outputs

The final project stage connects the complete execution chain with the research conclusion produced by its saved results.


### Complete Project Pipeline

The complete project can be executed through `src/Pipeline/run.py`.

The orchestrator runs the five executable stages in their required order:

1. Data System.
2. Factor Layer.
3. Factor Selection Layer.
4. General Research.
5. Low Volatility Research.

Every stage remains responsible for its own calculations, cache validation, outputs and reports. The complete pipeline does not duplicate financial logic, delete existing data or force unnecessary rebuilding. Execution stops immediately when one stage fails, preventing later layers from using incomplete results.


### **Pipeline/run.py**

#### PIPELINE_STAGES
- Stores the required execution order and the path of every layer pipeline.

#### format_duration
- Converts elapsed seconds into a readable hours, minutes and seconds value.

#### run_stage
- Runs one layer with the same Python environment used for the complete pipeline.
- Prints the current stage and its execution time.
- Stops when the layer script is missing or exits with an error.

#### run_complete_pipeline
- Runs all five stages sequentially.
- Allows every layer to reuse its own valid cache.
- Prints the duration of every completed stage and the complete project runtime.
- Does not contain data, factor, selection or portfolio logic.


### Testing and Verification

The project currently contains 43 automated tests covering the most consequential implementation rules.

- **Data System:** quality masks, unconfirmed and confirmed extreme returns, price gaps, availability, adjusted-price/volume compatibility, ticker retries and aliases, reused symbols, risk-free-rate reuse and deletion scope.
- **Factor and Selection Layers:** quality at both forward-return endpoints, one-day signal lag, separation of monotonic and tail effects, and acceptance of negative as well as positive monotonic structures.
- **Research Layer:** portfolio neutrality, complete-cross-section weights, beta neutralization, transaction costs, calendar phases, batched-engine equivalence, previous-day signal timing, point-in-time risk-free regimes, both walk-forward structures, decision-path consistency and neighbouring-window plateau logic.

These tests verify declared calculations and information timing. They cannot prove that an external data source is correct, that a historical anomaly will persist, or that the backtest represents achievable institutional execution.


### Output Reference

| Layer | Main readable result | Main reproducible data |
| --- | --- | --- |
| Data System | `Results/Data_System/data_audit_report.md` and `Figures/data_audit_summary.png` | `Data/Data_System/Raw` and `Data/Data_System/Processed` |
| Factor Layer | `Results/Factors_Layer/factor_layer_report.md` and `Figures/factor_layer_summary.png` | 56 factor matrices, eight forward-return matrices and `factor_metadata.csv` |
| Factor Selection | `Results/Factor_Selection_Layer/factor_selection_report.md` and `Figures/factor_selection_summary.png` | `daily_quantile_results.parquet`, `hypothesis_cards.csv`, `effect_tests.csv` and `quantile_curves.csv` |
| General Research | `Results/Research_Layer/General/research_report.md` and `Figures/general_research_summary.png` | Portfolio statistics, walk-forward results, risk diagnostics and reusable portfolio paths |
| Low Volatility Research | `Results/Research_Layer/Low_Volatility/low_volatility_report.md` and `Figures/low_volatility_summary.png` | Window summaries, portfolio statistics, walk-forward results and focused portfolio paths |

Large matrices and reusable caches are stored under `Data` and excluded from Git. Compact reports, figures, metadata and decision tables are stored under `Results` and retained in Git history.
